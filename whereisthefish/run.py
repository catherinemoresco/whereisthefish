...
overlay_screen = Screen(config['CONTROLLER']['HEIGHT'], config['CONTROLLER']['WIDTH'], 3, 3, [
  Button('z', cv2.imread("assets/images/b.png", 1)),
  Button('x', cv2.imread("assets/images/a.png", 1)),
  Button('w', cv2.imread("assets/images/up.png", 1)),
  Button('s', cv2.imread("assets/images/down.png", 1)),
  Button('a', cv2.imread("assets/images/left.png", 1)),
  Button('d', cv2.imread("assets/images/right.png", 1)),
  Button('q', cv2.imread("assets/images/select.png", 1)),
  Button('e', cv2.imread("assets/images/start.png", 1)),
  Button(None, cv2.imread("assets/images/empty.png", 1))
])
...
output = np.zeros((config['WINDOW']['HEIGHT'], config['CONTROLLER']['WIDTH'], 3), np.uint8)
output_stream_pipe = output.pipe
...
while True:
    ...
    if a!=-1 and b!=-1:
        ...
        if framecount == 15:
          button = overlay_screen.getButtonFromPosition(ccenter)
          ...
          to_prepend_width = int(math.floor(config['CONTROLLER']['WIDTH']/6))
        ...
        output[config['EMULATOR']['HEIGHT']-config['CONTROLLER']['HEIGHT']:config['EMULATOR']['HEIGHT'], config['CONTROLLER']['WIDTH']-config['CONTROLLER']['WIDTH']:config['CONTROLLER']['WIDTH']] = controller_frame
        ...
        topbar_bottom_left = (config['CONTROLLER']['WIDTH']-config['EMULATOR']['WIDTH'] , config['EMULATOR']['HEIGHT']-config['CONTROLLER']['HEIGHT'])
